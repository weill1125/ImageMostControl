...
namespace Com.Boc.Icms.LogDLL
{
    public sealed class FastLogger
    {
        ...
        */
        ...
        /// 日志
        /// </summary>
       private static FastLogger _fastLog=null;
        ...
        private FastLogger()
        {
            ...
            var configFile = new FileInfo(Path.Combine(Path.GetDirectoryName(Assembly.GetCallingAssembly().Location), "log4net.config"));


            if (!configFile.Exists)
            {
                 //   new Exception("未配置log4net配置文件！");
                return;
            }
            ...
            _mainlog = log4net.LogManager.GetLogger("MainLog");  //记录控件日志
            _sdklog = log4net.LogManager.GetLogger("SdkLog");  //记录Sdk日志            
          //  _eventlog = log4net.LogManager.GetLogger("EventLog");  //记录Sdk日志
        }
        ...
        private void EnqueueMessageFast(string message, FastLogLevel fastLogLevel = FastLogLevel.Info, FastLogType logType = FastLogType.MainLog, Exception ex = null, int eventId = 0)
        {
            try
            {
        ...
        private void WriteLog(FastLogMessage msg)
        {
            try
            {
                

                if (_mainlog != null && msg.LogType == FastLogType.MainLog)
                {
                    switch (msg.Level)
                    {
                        case FastLogLevel.Debug:
                            if (_mainlog.IsDebugEnabled)
                                _mainlog.Debug(msg.Message, msg.Exception);
                            
                            break;
                        case FastLogLevel.Info:
                            if (_mainlog.IsInfoEnabled)
                                _mainlog.Info(msg.Message, msg.Exception);
                            break;
                        case FastLogLevel.Error:
                            if (_mainlog.IsErrorEnabled)
                                _mainlog.Error(msg.Message, msg.Exception);
                            break;
                        case FastLogLevel.Warn:
                            if (_mainlog.IsWarnEnabled)
                                _mainlog.Warn(msg.Message, msg.Exception);
                            break;
                        case FastLogLevel.Fatal:
                            if (_mainlog.IsFatalEnabled)
                                _mainlog.Fatal(msg.Message, msg.Exception);
                            break;
                    }
                }else if (_sdklog != null && msg.LogType == FastLogType.SdkLog)
                {
                    switch (msg.Level)
                    {
                        case FastLogLevel.Debug:
                            if (_sdklog.IsDebugEnabled)
                                _sdklog.Debug(msg.Message, msg.Exception);

                            break;
                        case FastLogLevel.Info:
                            if (_sdklog.IsInfoEnabled)
                                _sdklog.Info(msg.Message, msg.Exception);
                            break;
                        case FastLogLevel.Error:
                            if (_sdklog.IsErrorEnabled)
                                _sdklog.Error(msg.Message, msg.Exception);
                            break;
                        case FastLogLevel.Warn:
                            if (_sdklog.IsWarnEnabled)
                                _sdklog.Warn(msg.Message, msg.Exception);
                            break;
                        case FastLogLevel.Fatal:
                            if (_sdklog.IsFatalEnabled)
                                _sdklog.Fatal(msg.Message, msg.Exception);
                            break;
                    }

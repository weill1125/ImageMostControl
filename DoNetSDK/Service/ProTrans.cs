...
namespace Com.Boc.Icms.DoNetSDK.Service
{
    ...
    class ProTrans
    {
        ...
        /// </summary>
        /// <returns>0表示成功</returns>
        [DllImport("ftpdllV01.0C.dll", CharSet = CharSet.Unicode)]
        ...
        internal bool GetFiles(string ftpIp, int ftpPort, string ftpRemotePath, string ftpLocalPath, string systemid, int ifWriteTxtLog, WriteLog writeStrLog, ref int ftpResult, ref int successSign,ref List<string> dlfailedFiles, bool ifDel = true)
        {
            ...
            try
            {
                ...
                if (int.Parse(CommonFunc.GetJsonNodeValue("//TotalFilesLength")) > 0)
                {
                    int totalFilesLength = int.Parse(CommonFunc.GetJsonNodeValue("//TotalFilesLength"));
...
*/
                                ...
                                while (fileInfosList.Count > 0)
                                {
                                    ...
                                    lock (Object)
                                    {
                                        if (fileInfosList.Count > 0)
                                        {
                                            ...
                                            pFtpRemoteFilePathName = pFtpRemotePath + fileName;
                                            fileInfosList.RemoveAt(0);
                                        }
                                        else
                                        {
        ...
        internal bool PutFiles(string ftpIp, int ftpPort, string ftpRemotePath, string ftpLocalPath, string systemid, int ifWriteTxtLog, WriteLog writeStrLog, ref string ftpResult, ref string failedFileName)
        {
            ...
            try
            {
                ...
                //取得pFtpLocalPath文件下所有文件包括子目录下的文件的fileinfo
                DirectoryInfo driveInfo = new DirectoryInfo(pFtpLocalPath);

                //获取本地文件数组
                files = driveInfo.GetFiles("*.*", SearchOption.AllDirectories);
                ...
                if (this.BeforeTransfer != null)
                {
                    this.InvokeBeforeTransfer(files.Select(file => file.Name).ToArray());
                }
                ...
                for (int i = 0; i < files.Length; i++)
                {
                    ...
                    //取文件名
                    string strFileName = files[i].Name;
                    pFtpLocalFilePathName = files[i].FullName;

                    pFtpRemoteFilePathName = Path.Combine( pFtpRemotePath , strFileName);
                    ...
                    Console.WriteLine(pFtpLocalFilePathName);
                    Console.WriteLine(pFtpRemoteFilePathName);
        ...
        internal int GetFile(string ftpRemotePath, string ftpLocalPath, string fileName, string systemid, int ifWriteTxtLog, WriteLog writeStrLog)
        {
            ...
            try
            {
                ...
                //设置远程文件路径及本地文件路径
                string pFtpRemoteFilePathName = Path.Combine(ftpRemotePath, fileName);
                string pFtpLocalFilePathName = Path.Combine(ftpLocalPath , fileName);
                //添加tmp文件，下载完成后再改过来
                string pFtpLocalFileTmpPathName = Path.Combine( ftpLocalPath, Guid.NewGuid() + "~tmp");
                ...
                else
                {
                    ...
                    //下载成功后把temp修改成要下的文件名
                    File.Move(pFtpLocalFileTmpPathName, pFtpLocalFilePathName,true);
        ...
        internal int SetXmlValue(string paraname, int id, int ifByteNum, int ifWriteTxtLog, WriteLog writeStrToLog)
        {
            ...
            try
            {
                timeout = int.Parse(CommonFunc.GetJsonNodeValue(paraname));

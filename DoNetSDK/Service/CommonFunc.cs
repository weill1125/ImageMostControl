...
namespace Com.Boc.Icms.DoNetSDK.Service
{
    public class CommonFunc
    {
        ...
        internal static string SendMessage(MessageBean message, string data, int ifWriteTxtLog, WriteLog writeStrToLog, string pid)
        {
            ...
            //if (message.ClientIp.Length < 15)
            //    message.ClientIp = message.ClientIp + spaceStr.Substring(0, 15 - message.ClientIp.Length);

            //if (message.Bankcode == "")
            //    message.Bankcode = "000";

            //if (message.Encode == "")
            //    message.Encode = "    ";

            //if (!string.IsNullOrEmpty(message.DataProvince))
            //{
            //    message.ExtData = message.DataProvince.PadRight(10, ' ');
            //}
            //else if (message.ExtData == "")
            //    message.ExtData = "          ";

            //if (message.Version == "")
            //    message.Version = "0301";
            ...
            if (int.Parse(GetJsonNodeValue("//ConnectTimeout")) > 0)
            {
                ConnectTimeout = int.Parse(GetJsonNodeValue("//ConnectTimeout"));
            }
            ...
            try
            {
                #region 启用keepAlive定时探测 windows 特定的，在linux下删除

                /*uint dummy = 0;
                int keepAliveTime =30000;

                if (int.Parse(GetJsonNodeValue("//keepAliveInterval")) > 0)
                {
                    keepAliveTime = int.Parse(GetJsonNodeValue("//keepAliveInterval"));
                } 
...
*/
                ...
                if (int.Parse(GetJsonNodeValue("//RcvMsgTimeout")) > 0)
                {
                    socket.ReceiveTimeout = int.Parse(GetJsonNodeValue("//RcvMsgTimeout"));
                }
                else
                {
                ...
                reciveCount = socket.Receive(lenBytes, 5, 0);
                WriteLog("reciveCount：" + reciveCount);
                if (reciveCount == 0)
                {
                ...
                //{
                //    WriteLog("IsLittleEndian：" + BitConverter.IsLittleEndian);
                //    Array.Reverse(lenBytes);
                //}
                  
                //lenBytes中长度为Data Field长度，还得增加RETURN_CODE长度

                string respLen = Encoding.UTF8.GetString(lenBytes);
                WriteLog("respLen：" + respLen);
                receriveLength = Convert.ToInt32(respLen);
                WriteLog("接收长度：" + receriveLength);
                //receriveLength = BitConverter.ToInt32(lenBytes, 0) + 5;
                ...
                while (socket.Connected)
                {
                    try
                    {
                        ...
                        WriteLog("iBytes：" + iBytes);
                        bytesCount += iBytes;
                        WriteLog("bytesCount：" + bytesCount);
                        //将字节数组中的一个字节序列解码为字符串
        ...
        internal static void WriteTxtLog(string className, string lineNo, string errorNo, string message, string pid)
        {
            ...
            SysLog.WriteLogFile(strLog, FastLogType.SdkLog);
        }
        ...
        /*internal static string GetXmlNodeValue(string nodename)
        {
            ...
            string path = Path.Combine(Path.GetDirectoryName(Assembly.GetCallingAssembly().Location), "SDK_Paras.xml");
            xmldoc.Load(path);
        ...
        internal static string GetJsonNodeValue(string nodename)
        {
            string result = "";
            string path = Path.Combine(GlobalFinalProperties.GlobalInstallPath, "SDK_Paras.json");
            StreamReader readerfile = File.OpenText(path);
            JsonTextReader reader = new JsonTextReader(readerfile);
            JObject sdkjson = (JObject)JToken.ReadFrom(reader);
            readerfile.Close();
            ...
            JObject paras = (JObject)sdkjson["paras"];
            if (paras.ContainsKey(nodename.Replace("//", "")))
            {
                result = paras[nodename.Replace("//", "")].ToString();
            }
        ...
        internal static string GetErrorMsg(string errorNo)
        {
            ...
            StreamReader file = new StreamReader(Path.Combine(GlobalFinalProperties.GlobalInstallPath, "Resource.json"), Encoding.UTF8);
            JObject jsonObject = (JObject)JToken.ReadFrom(new JsonTextReader(file));
            file.Close();
            JObject ResourceObject = (JObject)jsonObject["Resource"];

             result = ResourceObject["s" + errorNo].ToString();
        ...
        {
            //加密后文件临时存放路径
            string tempFilePath = Path.Combine(Path.GetDirectoryName(Assembly.GetCallingAssembly().Location), "temp");
